...
public class DataSaver<T> where T : new()
{
    ...
    public DataSaver(string fileName, bool encrypt = false, bool saveHistory = false){
        ...
        if(saveHistory){
            ...
            string aux = System.IO.Path.Combine(Application.persistentDataPath, fileName);
            this.config = System.IO.Path.Combine(aux, this.config);
